...
| 12 | Reseller Module | Partial | A real onboarding + verification + selling loop shipped 2026-08-04: `resellers` table (5-state `verification_status`) + RLS, `POST /api/resellers/apply` and an apply form on `/dashboard/reseller`, admin review queue (`GET /api/resellers`, `POST /api/resellers/:id/review` → `review_reseller()` RPC, which notifies the reseller and requires a note to reject/request changes), and a reseller dashboard with status banner, customer count, pending commission and wallet balance. Reseller checkout is gated on `approved` and on an attached customer, and now generates commission. No document/ID upload, no sub-reseller tiers, no per-reseller commission rates.
...
| 14 | Customer Module | Partial | Reseller-scoped CRM shipped 2026-08-04: `customers` table with RLS scoped through `resellers.user_id`, an immutability trigger on `customers.reseller_id`, `GET/POST /api/customers` (with name/phone search) and `PATCH /api/customers/:id`, plus a customer book panel and a search-or-create selection modal. Customers can be built while verification is still pending. No customer-facing accounts, addresses table, or order history per customer.
| 15 | Cart Module | Partial | `carts`/`cart_items` tables + RLS shipped 2026-08-01. `POST /api/cart/items` (add/increment), `GET /api/cart` (list with product join + computed subtotals), `DELETE /api/cart/items/:id` (remove) all work against real data; `/cart` page lists items and totals live. A reseller cart is now customer-gated (2026-08-04): `POST /api/cart/items` refuses an unattributed add with `code: "customer_required"`, which is what opens the selection modal; the chosen customer is stored on `carts.for_customer_id`, reused for the rest of the cart session, surfaced by `GET /api/cart` as `forCustomer`, and re-attributable via `PATCH /api/cart/customer` without clearing line items. No quantity-adjust-in-place, no wishlist. |
| 16 | Checkout Module | Partial | `/checkout` page + `POST /api/checkout` call a new `place_order()` Postgres function (`SECURITY DEFINER`, stock-checked, atomic) that creates the order, writes `order_items`, decrements `products.stock_int`, and marks the cart `converted` — verified end-to-end in the browser with real stock decrementing correctly. No address/delivery/payment-method steps, no `/api/v1` path convention. Reseller verification *is* now enforced (2026-08-04): `place_order()` takes only `p_customer_id` and derives reseller attribution from the caller's own profile, rejecting an unapproved reseller, a cart with no attached customer, or a customer belonging to another reseller — so the old `p_reseller_id: null` bug that made UI checkout unable to earn commission is gone. Merchant verification at checkout is still unenforced. |
