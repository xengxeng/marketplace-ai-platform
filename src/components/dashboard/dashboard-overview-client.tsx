...
import { ArrowDownRight, ArrowUpRight, DollarSign, PackageSearch, ShoppingBag, Store } from "lucide-react";
...
export type Metric = {
  ...
  changeLabel: string;
  up: boolean;
  icon: "revenue" | "orders" | "merchants" | "pending";
  glow: string;
};
...
const ICONS = { revenue: DollarSign, orders: ShoppingBag, merchants: Store, pending: PackageSearch };
    ...
    <div className="flex flex-col gap-6">
      ...
      >
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold text-white sm:text-4xl">Welcome back, {greetingName}</h1>
          <p className="mt-2 max-w-2xl text-sm text-zinc-400">
            Live figures from your marketplace database — merchants, orders, and revenue update as they happen.
          </p>
        </div>
        <Link
          href="/products"
            ...
            >
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`flex items-center gap-0.5 text-xs font-semibold ${metric.up ? "text-emerald-300" : "text-amber-300"}`}>
                  <Trend className="h-3.5 w-3.5" /> {metric.changeLabel}
                </span>
              </div>
        ...
        >
          ...
          <div className="mt-4 space-y-3">
            {activity.length === 0 ? (
              ...
              activity.map((entry) => (
                <div key={entry.title + entry.time} className="rounded-2xl border border-white/10 bg-black/20 p-3.5 transition hover:border-white/20">
